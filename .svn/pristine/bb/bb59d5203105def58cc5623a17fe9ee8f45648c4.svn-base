...
-(void)reset {
    ...
    [self updateSize];
    [self setNeedsDisplay];
    [delegate calendarView:self switchedToMonth:[currentMonth month] targetHeight:self.calendarHeight animated:NO andSwitchedToYear:[currentMonth year]];
}
...
-(void)showNextMonth {
    ...
    if ([delegate respondsToSelector:@selector(calendarView:switchedToMonth:targetHeight: animated:andSwitchedToYear:)]) [delegate calendarView:self switchedToMonth:[currentMonth month] targetHeight:self.calendarHeight animated:YES andSwitchedToYear:[currentMonth year]];
...
{       
    ...
    if (CGRectContainsPoint(rectArrowLeft, touchPoint)) {
        ...
        [self reset];
        if ((todayMonth!=currentMonthIndex) && [delegate respondsToSelector:@selector(calendarView:switchedToMonth:targetHeight:animated:andSwitchedToYear:)]) [delegate calendarView:self switchedToMonth:[currentMonth month] targetHeight:self.calendarHeight animated:NO andSwitchedToYear:[currentMonth year]];
    }
}
...
//    [labelCurrentMonth sizeToFit];
    ...
    for (int i =0; i<[weekdays count]; i++) {
        ...
        [weekdayValue drawInRect:CGRectMake(i*(kVRGCalendarViewDayWidth+2), 40, kVRGCalendarViewDayWidth+2, 20) withFont:font lineBreakMode:UILineBreakModeClip alignment:NSTextAlignmentCenter];
    }
...
-(id)init {
    ...
    if (self) {
        ...
        labelCurrentMonth.font = [UIFont systemFontOfSize:17];
